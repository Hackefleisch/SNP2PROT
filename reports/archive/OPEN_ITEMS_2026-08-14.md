...
| 8 | ~~`PROP1_R112Q`: keep, correct, or drop?~~ **DECIDED 2026-08-14: keep as deposited.** | Its name says R→Q; the deposited sequence gives R→M. Kept with bookkeeping taken from the sequence. `PITX2_T114P` remains excluded (its insert shows no substitution at all). The other two anomalies vanished when their genes were rejected as mixed-family.

**Detail.** The name says residue 112 Arg->Gln; the deposited sequence gives Arg->**Met** at that same position. The position and the wild-type residue are both correct — only the replacement disagrees. PROP1's other allele, `R99Q`, matches its name exactly, so the discrepancy is specific to this clone. Both Arg->Gln and Arg->Met are single-nucleotide changes from an Arg codon, and the Barrera supplement lists R99Q but **not** R112Q, so nothing available settles which the clone really was.

**It matters more than most.** Construct position 57 maps to **homeodomain position 43 — the first residue of helix 3, the recognition helix** that reads bases in the major groove. R99Q by contrast sits at homeodomain position 30, in the loop before helix 2.

**Kept as deposited.** The allele name is stored nowhere in the schema: `dbd_seq` is the sequence, and `mut_positions` (53, within the padded domain) is derived by diffing against the reference, never parsed from the name. So the stored claim — "this domain, with M at position 53, binds these 8-mers" — is true whichever name is right. **The one exposure is a future join on allele name `R112Q` that assumes Gln; anyone doing that must read this row first.** | **closed** |
...
| 54 | **Investigate whether the dataset build can be sped up.** `build_dataset.py --all` takes ~25 minutes, which makes every method change expensive to validate and discourages iteration. Nothing has been measured yet; likely candidates are the per-source Pfam-A scan (30,134 profiles loaded per source rather than once), re-reading large archives, and the absence of any caching of domain calls between runs. Worth profiling before optimising. | not started | **medium — it gates iteration speed** |
| 55 | **18 domains carry `species` as the literal string `$species`** — an unsubstituted template placeholder on some UniPROBE detail pages, not a parsing fault on our side. Species is metadata only and affects no label, but it should be resolved (UniProt carries the organism) or normalised to null. Related: the same field records both `C. elegans` and full binomials, so organism names are not consistently formatted. | open | low |

| 56 | **UniPROBE is not the only PBM repository — CIS-BP holds substantially more.** CIS-BP build 3.10 reports **2,294 TFs with PBM data** (plus 97 under `PBM,_CSA_and_or_DIP-chip`), against 4,989 TFs with any directly determined motif, over 741 species. Its bulk download page serves **E-scores, Z-scores and probe intensities** per species — the same data type we use, not just PWMs — through a POST form to `bulk_archive.php`. Overlap with our 779 UniPROBE constructs is unknown and probably large, since CIS-BP aggregates published data including Bulyk-lab deposits. **Before pursuing: check whether CIS-BP publishes the assayed construct sequence.** Our admission policy rests on annotating what was physically on the array; if CIS-BP supplies only its own DBD annotation of the full-length protein, attributability cannot be established and the data cannot be admitted under the current conditions. That single question decides whether this is a large expansion or a dead end. | open | **medium — the largest identified expansion** |
| 57 | **Other PBM sources not yet surveyed**: individual GEO / ArrayExpress deposits and paper supplements. Lower yield per unit effort than #56 and with the same construct-sequence caveat. | not started | low |

| 58 | **`Path10` publishes truncated 8-mer tables and is excluded.** Its files carry a valid E-score column, but only 341-1,391 rows each, cut at E >= 0.25, rather than the full 32,896. There are no negatives in it at all, and applying our cutoffs would label the enriched 8-mers between 0.25 and 0.35 as non-binding — the exact inverse of the truth. 18 *Plasmodium* AP2 proteins excluded; deregistered, with the reason in `PROVENANCE.md`. The reader now distinguishes this case ("an E-score column that never goes negative") from a file with no E-score, and separately requires a complete 32,896-row table. | fixed |

| 59 | **`GD09` excluded, twice over.** Its tables hold 32,895 of the 32,896 non-redundant 8-mers (the palindrome `CCTTAAGG` is missing), and the fully crossed design means a protein measured against a different 8-mer set cannot join the others. Independently, its single protein NSY-7 — deposited twice, as `NSY-7` and `Nsy-7` — returns no Pfam hit anywhere in its 294-residue construct. The completeness error now distinguishes "truncated to the enriched end" (Path10, ~4% of the set) from "near-complete but *n* short" (GD09, one short), so a future source in either state gets an accurate message. | fixed |
