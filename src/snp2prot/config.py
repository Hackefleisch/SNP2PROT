...
#: Trained two-tower weights, one file per (arm, fold). Written by `snp2prot.training.run_fold`
#: and logged to MLflow alongside the run. Distinct from `MODEL_DIR`, which holds third-party
#: checkpoints the build *depends on*: these are outputs, regenerated by `scripts/run_grid.py`.
